		...
		if (get_capture_result == K4A_WAIT_RESULT_SUCCEEDED) {
			...
			if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED) {
				...
				for (int i = 0; i < num_bodies; i++) {
					...
					else if ((visitor[id - 1].pre_R_hand_joint_z <= 500 && visitor[id - 1].skeleton.joints[K4ABT_JOINT_HAND_RIGHT].position.xyz.z >= 500) ||
						(visitor[id - 1].pre_L_hand_joint_z <= 500 && visitor[id - 1].skeleton.joints[K4ABT_JOINT_HAND_LEFT].position.xyz.z >= 500)) {
