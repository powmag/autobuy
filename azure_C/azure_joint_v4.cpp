...
// v3는 두 사람 찍혔을 때 제대로 동작
// v4는 두 사람 이상일 때 제대로 돌아가는지 아직 모름
/***********************************************/
		...
		if (get_capture_result == K4A_WAIT_RESULT_SUCCEEDED) {
			...
			if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED) {
				...
				for (int i = 0; i < num_bodies; i++) {
					...
					if ((visitor[id - 1].skeleton.joints[K4ABT_JOINT_HAND_RIGHT].position.xyz.z <= 500 ||
						visitor[id - 1].skeleton.joints[K4ABT_JOINT_HAND_LEFT].position.xyz.z <= 500) && frame_take == 0) {
					...
					else if ((visitor[id - 1].pre_R_hand_joint_z <= 700 && visitor[id - 1].skeleton.joints[K4ABT_JOINT_HAND_RIGHT].position.xyz.z >= 5700) ||
						(visitor[id - 1].pre_L_hand_joint_z <= 700 && visitor[id - 1].skeleton.joints[K4ABT_JOINT_HAND_LEFT].position.xyz.z >= 700)) {
