...
char* right_img_capture(k4a_capture_t cap, char* temp, char *directory, char* nn, char* ex);
char* left_img_cap(char* temp, char* directory, char* nn, char* ex);
		...
		if (get_capture_result == K4A_WAIT_RESULT_SUCCEEDED) {
			...
			if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED) {
				...
				for (int i = 0; i < num_bodies; i++) {
					...
					if (visitor[id - 1].pre_R_hand_joint_x <= 0 && visitor[id - 1].skeleton.joints[K4ABT_JOINT_HAND_RIGHT].position.xyz.x >= 0) {
						// 이전 프레임이랑 비교해서 오른손 빠져나갈 때 캡처

						// 캡처
						cap_name = right_img_capture(sensor_capture, time_string, dir, new_name, extention);
					...
					else if (visitor[id - 1].pre_L_hand_joint_x <= 0 && visitor[id - 1].skeleton.joints[K4ABT_JOINT_HAND_LEFT].position.xyz.x >= 0) {
						// 이전 프레임이랑 비교해서 왼손 빠져나갈 때 캡처

						// 캡처
						cap_name = left_img_cap(time_string, dir, new_name, extention);
...
char* left_img_cap(char* temp, char* directory, char* nn, char* ex) {
	...
	// 저장
	save_result = imwrite(directory, frame);

	if (save_result == 1)
		printf("왼쪽 이미지 저장 success\n");
	else
		printf("왼쪽 이미지 저장 fail\n");
	cap.release();
...
*/	
	...
	Rect rect(source_point3d->xyz.x + 1800, source_point3d->xyz.y + 1400, 500, 500);
